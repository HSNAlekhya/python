...
-> Variables 
 ...
 -> Interpolation : 
           ...
           Replacing Placeholder with actual Values dynamically
...
-> Control Structures / Statements
    ...
    -> Looping Statements (Iteration Statements)
        ...
        -> for loop : used to iterate over a sequence (__iter __)
...
-> Functional Programming
    ...
    -> Why Functions ?
        ...
        -> Functions can help in breaking down complex problems into simpler and smaller manageable sub tasks

        -> You can call a function multiple times, which avoid Code Repetition, which promotes Code Reusability 
  ...
  -> Arguments to function 
    ...
    -> Positional Arguments 
        ...
        -> These are passed to functions in the exact order as defined in function signature 

        -> Order/Position matters, the first argument you passed goes to first Parameter, second argument you passed goes to second Parameter and so on

    -> Keyword  Arguments

        -> Arguments are passed explicitly by specifying the name, so here function call is based on name of argument not Position
    ...
    -> Default Arguments
        ...
        -> If we pass any value to the default argument at the function call, default value will be overridden with new value passed
   ...
   -> Scope Of Variables inside Functions
    ...
    -> Global Scope 
        ...
        -> As global variables can be accessed within the functions, there can be  name conflicts, in this case first preference is given to local variables 

            NOTE: To access global variables, use globals()

        -> If you want global variables to be changed inside the function, then we  need to use a special keyword called global 
 ...
 -> Modules 
    ...
    -> Syntax 
        ...
        from module import specific functionality (import only specific Functionalities) (recommended)
...
-> 4 Pillars Of OOP 
 ...
 -> Encapsulation 
        ...
        -> Encapsulation is a core concept in object-oriented programming (OOP) that binds
         data (attributes) and the methods that operate on that data into a single unit, 
         like a class

        -> This process involves restricting direct access to an object's internal state, 
            which is a principle known as "data hiding" 

        -> Other Languages like Java or C++ enforces Strict Rules for Encapsulation,  
            coming to python there is no enforcing 

            -> Java/C++ = “You’re not allowed.” 
            -> Python = “You shouldn’t, but you can if you insist.”

        -> While Python does not have strict access modifiers like private or protected 
        keywords found in some other languages, 
        python uses "naming conventions" to indicate the intended visibility of members.
        ...
        -> Hide Data 
            ...
            -> We have Public 
                ...
                -> Without any special prefix are considered public and 
                    can be accessed from anywhere

            -> We Have Private 

                -> prefixed with a double underscore (__) trigger

                -> Cannot be accessed from outside

                -> print(obj._MyClass__private_variable) # Accessing through mangled name is possible
 
            
            -> We Have Protected 

                -> prefixed with a single underscore (_) trigger

                -> This signals to other developers that the attribute is intended for 
                internal use within the class and its subclasses, 
                and should not be directly accessed or modified from outside the class.

        -> Getters & Setters 

            -> In Python, getters and setters are methods used to control access to 
            class attributes, promoting data encapsulation and allowing for validation 
            when retrieving or modifying attribute values. 
            
        -> @property

            -> While Python doesn't enforce strict "private" attributes like 
            some other object-oriented languages, the use of getters and setters, 
            often with the @property decorator, 
            provides a Pythonic way to achieve this control.

-> Abstraction 

        -> Abstraction in OOP is a principle that involves hiding complex implementation 
        details and showing only the essential features of an object to the user. 
        It simplifies complex systems by breaking them into manageable components, 
        making code more intuitive, reusable, and easier to maintain. 
        
        -> Abstraction is achieved through abstract classes and interfaces, 
            which define a blueprint without exposing the underlying code.

        -> Hiding implementation: Abstraction hides the internal workings of an object 
        and presents a simplified view. For example, when using an ATM, you only 
        interact with a simple interface (card, PIN, cash dispenser) without needing 
        to know the complex process of backend banking operations that occur. 

        -> Providing an interface: It defines a contract 
        (using abstract classes or interfaces) that specifies 
        what a class can do, without dictating how it does it. 

        -> Ensuring security: By controlling what is exposed, 
        abstraction protects the internal data and logic from unintended 
        access or misuse. 

        -> How To Achieve Abstraction In python 

            -> An abstract class in Python functions as a blueprint for other classes, 
            defining a common interface or structure that subclasses must adhere to. 
            It cannot be instantiated directly, meaning you cannot create objects 
            of an abstract class. 
            
            -> Abstract classes are implemented in Python using the 
                abc module (Abstract Base Classes)

                ->  from abc import ABC, abstractmethod
            
                -> Create a class that inherits from ABC. 
                Inside this class, define one or more abstract methods using 
                the @abstractmethod decorator. 
                Abstract methods are declared but do not have an implementation 
                (they typically contain pass).

            -> Implement Subclasses: Create concrete subclasses that inherit from the 
            abstract class. These subclasses must implement all abstract methods defined 
            in the abstract parent class. 
            If a subclass fails to implement any abstract method, 
